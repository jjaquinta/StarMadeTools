...
public class BlockTypeColors
{
    ...
    public static void loadBlockIcons()
    {
        ...
        try
        {
            ...
            InputStream is = ResourceUtils.loadSystemResourceStream("data/config/BlockTypes.properties", LegacyResources.class);
            mBlockTypes.load(is);
            is.close();
            Document doc = XMLUtils.readStream(ResourceUtils.loadSystemResourceStream("data/config/BlockConfig.xml", LegacyResources.class));
	...
	private static void loadTextureMaps() throws IOException
	{
		for (int i = 0; i < 256; i++)
		{
		    String fname = "t"+StringUtils.zeroPrefix(i, 3)+".png";
		    InputStream is = ResourceUtils.loadSystemResourceStream("data/textures/block/Default/128/"+fname, LegacyResources.class);
		    if (is == null)
		        break;
